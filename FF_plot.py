...
if __name__ == "__main__":
  ...
  DRsr_region_ditau = "(HTT_pdgId > 0) & (METfilters) & (LeptonVeto==0) & (abs(HTT_pdgId)==15*15) & (Trigger_ditau)"
  DRar_region_ditau = "(HTT_pdgId > 0) & (METfilters) & (LeptonVeto==0) & (abs(HTT_pdgId)==15*15) & (Trigger_ditau)"
  ...
  for process in file_map: 
    ...
    cut_events_AR = new_process_dictionary[process]["info"]
    cut_events_AR = apply_AR_cut(process, cut_events_AR, final_state_mode, jet_mode, DeepTau_version,
                                 determining_FF = True)
    ...
    if "DY" in process:
      event_flavor_arr = cut_events_AR["event_flavor"]
      ...
      background_gen_deepcopy = copy.deepcopy(cut_events_AR)
      ...
      background_lep_deepcopy = copy.deepcopy(cut_events_AR)
      ...
      background_jet_deepcopy = copy.deepcopy(cut_events_AR)
      ...
      combined_process_dictionary = append_to_combined_processes(process, cut_events_AR, vars_to_plot, 
                                                                 combined_process_dictionary)
