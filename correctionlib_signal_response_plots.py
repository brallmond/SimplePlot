...
if __name__ == "__main__":
  ...
  #signal_processes = ["ggH_TauTau", "VBF_TauTau", "WpH_TauTau"]
  ...
  process_colors = {"ggH_TauTau" : "blue", "VBF_TauTau" : "red", "WpH_TauTau" : "green"}
  process_labels = {"ggH_TauTau" : "ggH", "VBF_TauTau" : "qqH", "WpH_TauTau" : "WpH"}
  ...
  for process in signal_processes: 
    ...
    do_normed_diff_comparison = False
    ...
    if (do_normed_diff_comparison):
      ...
      for i in range(len(binning)):
        ...
        print("DEBUG")
        print(max(reco_gen_normed_diff_hist))
    ...
    if (do_correction == True):
      ...
      reco_gen_ratio_vals[np.isnan(reco_gen_ratio_vals)] = 1.0 # sets nans to ones, nans are from no events in bin
      ...
      # output values to terminal and collect manually in correctionlib setter file 
      print(process)
      print("[", end="")
      for i, val in enumerate(list(reco_gen_ratio_vals)):
        temp = f"{val:.3f}"
        temp += " " if (i==(len(reco_gen_ratio_vals)-1)) else ", "
        print(temp, end="")
      print("]")
